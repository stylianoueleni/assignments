...
public class Stratiotis extends Pioni
{
     ...
     */
    Stratiotis(boolean isWhite)
    {
         super(isWhite,(isWhite)?'P':'p');
    }
    ...
    public boolean isLegalMove(char x, int y, ChessBoard b){
       ...
       if(yCoord==2 || yCoord==7){
           if(x1==x2 && Math.abs(y-yCoord)==2 && b.board[8-y][x2-1]==null && b.board[8-y-1][x2-1]==null){
               return true;
           }
       }
       if(x1==x2 && Math.abs(y-yCoord)==1 && b.board[8-y][x2-1]==null){
           return true;
       }
       else if((Math.abs(x1-x2)==1) && Math.abs(y-yCoord)==1){
